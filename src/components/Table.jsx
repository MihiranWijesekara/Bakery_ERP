...
import { FiSearch, FiChevronUp, FiChevronDown, FiDownload, FiCheckSquare, FiSquare, FiMoreVertical } from 'react-icons/fi';
...
export const Table = ({ 
  ...
  const processedData = useMemo(() => {
    ...
    if (selectedFilter && filterField) {
      result = result.filter(item => {
        ...
        return val && val.toString().toLowerCase() === selectedFilter.toLowerCase();
      });
    }
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute("download", `${title ? title.toLowerCase().replace(/\s+/g, '_') : 'table_data'}_export.csv`);
  ...
  return (
    <div className="table-container">
      ...
      {selectedRows.size > 0 && bulkActions && (
        <div style={{ padding: '10px 20px', backgroundColor: 'var(--primary-light)', borderBottom: '1px solid var(--surface-border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontSize: '12px', fontWeight: 600, color: 'var(--primary)' }}>
            {selectedRows.size} row(s) selected
          </span>
          <div style={{ display: 'flex', gap: '8px' }}>
            {bulkActions(Array.from(selectedRows), () => setSelectedRows(new Set()))}
          </div>
        </div>
      )}
        ...
        <table className="enterprise-table">
            ...
            <tr>
              ...
              {bulkActions && (
                <th style={{ width: '40px', textAlign: 'center' }}>
                  <span onClick={handleSelectAll} style={{ cursor: 'pointer', display: 'flex', justifyContent: 'center' }}>
          ...
          <tbody>
            {paginatedData.length === 0 ? (
              <tr>
                <td colSpan={columns.length + (bulkActions ? 1 : 0)} style={{ textAlign: 'center', padding: '30px', color: 'var(--text-muted)' }}>
                  No records matching the filters were found.
                </td>
              </tr>
            ) : (
              paginatedData.map((row, rIndex) => (
                <tr key={row.id || rIndex} className={selectedRows.has(row.id) ? 'selected-row' : ''}>
                  {/* Select Row Checkbox */}
                  {bulkActions && (
                    <td style={{ textAlign: 'center' }}>
                      <span onClick={() => handleSelectRow(row.id)} style={{ cursor: 'pointer', display: 'flex', justifyContent: 'center' }}>
                  ...
                  {columns.map(col => {
                    ...
                    return (
                      <td key={col.accessor}>
                        {col.cell ? col.cell(row) : (cellValue !== undefined ? cellValue : '-')}
                      </td>
                    );
                  })}
                </tr>
